...
import { CARRY_FLOOR_FRACTION, capperMonths } from "./cap-conversion"
...
export async function loadPlayerProfile(player: Player, allPlayers: Player[]): Promise<PlayerProfileData> {
  ...
  // Month-resolved crests (Wesley's Prodigy) can't be read off a single match
  // row, so they're resolved separately and merged in. This MUST stay in step
  // with the same merge in buildHistoryIndex (lib/achievements-server.ts): the
  // vault renders holders from there, this renders them here, and if the two
  // disagree a crest shows on one surface and not the other.
  for (const [id, holder] of resolveMonthlySecretHolders(
    capperMonths(playable, stats, killPairs, new Date()),
  )) {
