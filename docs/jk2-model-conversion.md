...
Two per-slot annotations ride along when a skin needs them, both generated by
consulting the same `.shader` scripts the graft reads (`scripts/jk2-shaders.mjs`):
`formats` marks a slot that must ship as PNG because its surface is alpha-tested
and a JPEG would silently discard the cutout (Bones' red/blue); `additive` marks
a slot whose target shader's base stage is `blendFunc GL_ONE GL_ONE` — no opaque
pass at all — which the viewer rebuilds as an additive emissive material rather
than a diffuse repaint (Andromeda's team-skin arm, her signature look).
...
`cross(v1-v0, v2-v0)` pointing the same way — so it counts the agreements PER
SURFACE and prints the verdict (`10/10 surfaces reversed, lowest agreement
62.7% on "r_hand"`). This matters because these materials are exported
double-sided: get the winding wrong and you don't see holes, you see lighting
that's subtly inverted everywhere, which nobody spots until it's uploaded.

Per surface because of horseton: 77% whole-model agreement, well under the old
global 90% bar — yet every one of its surfaces votes the same way. Smoothed
vertex normals on rounded organic shapes lean away from their faces' planes and
misfire the dot test triangle by triangle without ever changing a surface's
majority. Base-game models sit at 94–100% per surface, so the per-surface
verdicts reproduce every previous conversion byte-for-byte; the hard refusal
only fires when a surface is a near coin flip (under 55%), which is what
actually-misread geometry looks like. Low agreement is a *warning sign*, not
proof of a problem — the arbiter is the visual check in `/lab/model`.
...
| Grafted model is lit oddly from every angle, no obvious holes | Inverted winding. `glm-graft.mjs` prints its per-surface verdicts and the lowest agreement — high disagreement usually means messy smoothed normals (horseton bottomed out at 62.7% and converted fine), but if a surface renders inside out, its majority genuinely lost. |
