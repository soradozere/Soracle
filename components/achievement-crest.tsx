...
// strip (components/achievements-strip.tsx), the /achievements grid, and each
// achievement's own page. Presentation-only — it renders whatever AchievementView
// it is handed and owns no data logic.
//
// Each earned tier has one tell, escalating with rarity:
...
export interface CrestProps extends React.HTMLAttributes<HTMLDivElement> {
  a: AchievementView
  // The profile strip tracks one player's climb, so it shows the progress bar and
  // the "80 / 100 → III" label. The community-facing pages describe the crest
  // itself, where one visitor's progress would be meaningless.
  showProgress?: boolean
}

// forwardRef so Radix's <TooltipTrigger asChild> can attach to the tile.
export const Crest = forwardRef<HTMLDivElement, CrestProps>(function Crest(
  { a, showProgress = true, ...rest },
  ref,
) {
  ...
  return (
    <div ref={ref} className={classes} style={{ "--rc": color } as React.CSSProperties} {...rest}>
      {glows && (
      ...
      <div className="ach-face">
        ...
        {showProgress && a.progressPct != null && (
...
// Drop once per page that renders crests.
export function CrestStyles() {
  return <style>{ACH_CSS}</style>
}

export const ACH_CSS = `
