    ...
    <div className="relative z-10">
      ...
      <header className="mb-7">
        ...
        <p className="text-[#8892a0] max-w-[62ch] text-sm md:text-base">
          Every achievement in the game, who holds it, and who got there first. Ranks stack — a tiered achievement levels up as you
      ...
      {claimedSecrets.length > 0 && (
        <>
          ...
          <p className="text-[#8892a0] text-sm max-w-[70ch] mb-3">
            {claimedSecrets.length === 1 ? "One secret achievement has" : `${claimedSecrets.length} secret achievements have`} been
      ...
      {sealedSecrets > 0 && (
        <>
          ...
          <p className="text-[#8892a0] text-sm max-w-[70ch] mb-3">
            {sealedSecrets === 1 ? "One secret achievement exists" : `${sealedSecrets} secret achievements exist`} that exactly one
...
.ach-count{flex:1 1 150px;background:color-mix(in srgb, var(--color-surface) 62%, var(--color-background));border:1px solid color-mix(in srgb, var(--color-border) 72%, var(--color-background));border-radius:8px;padding:12px 14px}
.ach-count b{display:block;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:24px;font-variant-numeric:tabular-nums;color:var(--color-primary)}
.ach-count span{font-size:10.5px;text-transform:uppercase;letter-spacing:.12em;color:var(--color-text-dim);font-weight:700}
...
.ach-feed{list-style:none;margin:0;padding:0;border:1px solid color-mix(in srgb, var(--color-border) 72%, var(--color-background));border-radius:8px;background:color-mix(in srgb, var(--color-surface) 62%, var(--color-background));max-height:420px;overflow-y:auto;overscroll-behavior:contain;scrollbar-width:thin;scrollbar-color:color-mix(in srgb, var(--color-border) 72%, var(--color-background)) var(--color-background)}
.ach-feed::-webkit-scrollbar{width:6px}
.ach-feed::-webkit-scrollbar-track{background:var(--color-background);border-radius:999px;margin:4px 0}
.ach-feed::-webkit-scrollbar-thumb{background:color-mix(in srgb, var(--color-border) 72%, var(--color-background));border-radius:999px;border:1px solid var(--color-background)}
.ach-feed::-webkit-scrollbar-thumb:hover{background:var(--color-border)}
.ach-feed-row{display:grid;grid-template-columns:34px 1fr auto auto;gap:12px;align-items:center;padding:10px 14px;border-top:1px solid color-mix(in srgb, var(--color-border) 72%, var(--color-background));color:var(--color-text);text-decoration:none}
.ach-feed li:first-child .ach-feed-row{border-top:0}
.ach-feed-row:hover{background:color-mix(in srgb, var(--color-surface) 88%, var(--color-background))}
.ach-feed-row:focus-visible{outline:none;background:color-mix(in srgb, var(--color-surface) 88%, var(--color-background));box-shadow:inset 3px 0 0 var(--color-primary)}
...
.ach-feed-sub{font-size:11px;color:var(--color-text-dim)}
...
.ach-feed-date{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;color:var(--color-text-dim);font-variant-numeric:tabular-nums;white-space:nowrap}

.ach-filters{display:flex;gap:7px;flex-wrap:wrap;margin-bottom:6px}
.ach-filters button{font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;padding:6px 12px;border-radius:999px;border:1px solid var(--color-border);background:transparent;color:var(--color-text-dim);cursor:pointer;transition:color .15s,background .15s}
.ach-filters button:hover{color:var(--color-text-bright)}
.ach-filters button[aria-pressed="true"]{background:var(--color-primary);border-color:var(--color-primary);color:var(--color-background)}
.ach-filters button:focus-visible{outline:2px solid var(--color-primary);outline-offset:2px}
...
.ach-card:focus-visible{outline:2px solid var(--color-primary);outline-offset:6px;border-radius:8px}
.ach-card-meta{font-size:10px;letter-spacing:.1em;text-transform:uppercase;font-weight:700;color:var(--color-text-dim)}
.ach-card-meta[data-held="n"]{color:color-mix(in srgb, var(--color-text-dim) 70%, var(--color-background))}
