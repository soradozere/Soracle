...
export default function PlayerLoginPage() {
  ...
  // Starts true so an already-logged-in visitor never sees the form flash
  // before the redirect below fires - checked once per page load, same cost
  // as /player/[slug] already pays to know whether to show owner controls.
  ...
  useEffect(() => {
    ...
    fetch("/api/player-auth/me")
      ...
      .then((data) => {
        if (cancelled) return
        if (data.playerId) {
          router.replace(`/player/${playerSlug(data.name)}`)
        } else {
    ...
    <div className="min-h-screen bg-[#0b0c10] px-4 py-6">
      ...
      <div className="max-w-sm mx-auto relative z-10">
        ...
        <div className="bg-[#1f2833]/40 border border-[#3d4855] rounded-lg backdrop-blur-lg p-6">
          {checkingSession ? (
            <div className="flex items-center justify-center gap-2 text-sm text-[#8892a0] py-8">
